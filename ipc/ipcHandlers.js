...
export function setupIpcHandlers(
  ...
  ipcMain.on(
    'disconnect-channel',
    handleDisconnectChannel(disconnectFromChannel, pusher)
  )
...
function handleConnectChannels(
  ...
  return async (event, channels) => {
    try {
      ...
      await disconnectReconnectChannels(
        reconnectChannels,
        disconnectFromChannel,
        pusher
      )
...
function categorizeChannels(channels) {
  ...
  for (const channel of channels) {
    if (
      Array.from(global.activeChannels.values()).some(
        (ch) => ch.channelName === channel
      )
    ) {
...
async function disconnectReconnectChannels(
  reconnectChannels,
  disconnectFromChannel,
  pusher
) {
...
async function connectAllChannels(
  ...
  if (allChannels.length > 0) {
    ...
    const channelPairs = allChannels.map((channel, index) => [
      channel,
      chatroomIds[index],
    ])
...
function handleDisconnectChannel(disconnectFromChannel, pusher) {
  return async (event, channel) => {
    try {
      const channelEntry = Array.from(global.activeChannels.entries()).find(
        ([_, value]) => value.channelName === channel
      )
      if (channelEntry) {
        await disconnectFromChannel(channel, global.activeChannels, pusher)
      } else {
