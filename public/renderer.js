...
function createBadgeHTML(badge, channelSubscriberBadges) {
  const badgeHTML = (svg) =>
    `<span style="width: ${BADGE_SIZE}; height: ${BADGE_SIZE};">${svg}</span>`
  ...
  switch (badge.type) {
    case 'subscriber':
      ...
      if (matchingBadge) {
        ...
        return badgeHTML(
          `<img src="${matchingBadge.badge_image.src}" style="width: ${BADGE_SIZE};" alt="${badgeInfo}" title="${badgeInfo}" />`
        )
      }
      return ''
    case 'moderator':
      return badgeHTML(
        `<svg version="1.1" x="0px" y="0px" viewBox="0 0 16 16" title="Moderatör" xml:space="preserve" width="${BADGE_SIZE}" height="${BADGE_SIZE}"><path d="M11.7,1.3v1.5h-1.5v1.5H8.7v1.5H7.3v1.5H5.8V5.8h-3v3h1.5v1.5H2.8v1.5H1.3v3h3v-1.5h1.5v-1.5h1.5v1.5h3v-3H8.7V8.7h1.5V7.3h1.5V5.8h1.5V4.3h1.5v-3C14.7,1.3,11.7,1.3,11.7,1.3z" style="fill: rgb(0, 199, 255);"></path></svg>`
      )
    case 'founder':
...
ipcRenderer.on('message', (event, message) => {
  ...
  if (channelChatDiv) {
    ...
    if (Array.isArray(message.badges)) {
      badgesHTML = message.badges
        .map((badge) => createBadgeHTML(badge, channelSubscriberBadges))
        .join('')
    }
    ...
    const messageWith7TVEmotes = (msg) => {
      ...
      const msg_replaced = words
        .map((word) => {
          const finded = channelEmotes7tv.emote_set.emotes.find(
            (emote) => emote.name === word
          )
          if (finded) {
            const imgSrc =
              finded.data.host.url +
              '/' +
              (finded.data.host.files?.[0]?.name || '1x.avif')
    ...
    const autoScroll =
      channelChatDiv.getAttribute('data-auto-scroll') === 'true'
    ...
    switch (message.status) {
      case 'connecting':
        ...
        if (createdTabChannelData.tab_active) {
          console.log(
            `Connecting to [tab name]: ${message.channel} [tab active].`
          )
        } else {
          console.log(
            `Connecting to [tab name]: ${message.channel} [tab not active].`
          )
        }
        existingChannels.set(message.channel, {
          ...message.channelData,
          tab_active: createdTabChannelData.tab_active,
        })
        break
      case 'connected':
...
function handleChatEvent(eventType, data) {
  ...
  switch (eventType) {
    case 'message-deleted':
      const messageDiv = channelChatDiv.querySelector(
        `div[data-message-id="${data.message.id}"]`
      )
      if (messageDiv) {
        messageDiv.classList.add('!opacity-40')
        messageDiv.setAttribute('data-deleted', 'true')
        const messageContainerDiv =
          messageDiv.querySelector('#message-container')
...
ipcRenderer.on('message-deleted', (event, data) =>
  handleChatEvent('message-deleted', data)
)
ipcRenderer.on('user-banned', (event, data) =>
  handleChatEvent('user-banned', data)
)
ipcRenderer.on('user-unbanned', (event, data) =>
  handleChatEvent('user-unbanned', data)
)
ipcRenderer.on('chatroom-cleared', (event, data) =>
  handleChatEvent('chatroom-cleared', data)
)
ipcRenderer.on('subscription-event', (event, data) =>
  handleChatEvent('subscription-event', data)
)
...
addChannelButton.addEventListener('click', () => {
  ...
  function handleClickOutside(event) {
    if (
      !channelModal.contains(event.target) &&
      !addChannelButton.contains(event.target)
    ) {
...
function addChannel() {
  ...
  if (channel) {
    if (existingChannels.has(channel)) {
      ...
      console.log(
        `Channel ${channel} not found in the tab menu, creating a new tab...`
      )
    }
    channelModal.classList.add('hidden')
    addChannelButton.classList.remove('hidden')
...
function createChannelTab(channel) {
  ...
  channelChatContainer.className =
    'grid grid-rows-[1fr_auto] w-full h-full hidden'
  ...
  scrollDownButton.className =
    'scroll-down-button hidden absolute bottom-11 w-full bg-black/70 text-white py-1.5 text-xs'
  ...
  channelChatDiv.addEventListener('scroll', () => {
    if (
      channelChatDiv.scrollTop + channelChatDiv.clientHeight <
      channelChatDiv.scrollHeight - 40
    ) {
  ...
  messageInputContainer.className =
    'w-full bg-[--box-color] border-t border-gray-700 flex items-center'
  const messageInput = document.createElement('input')
  messageInput.type = 'text'
  messageInput.id = 'message-input-' + channel
  messageInput.className =
    'flex-grow text-white bg-[--box-color] p-2 py-3 focus:outline-none'
...
function sendMessage(channel, message) {
  ...
  console.log(`Message to send: ${message} to channel: ${channelData.slug}`, {
    message,
    channel: channelData.slug,
  })
  const chatMessageSendInput = document.getElementById(
